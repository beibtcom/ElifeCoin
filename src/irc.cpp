...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("ElifeCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ElifeCoinTEST2\r");
            Send(hSocket, "WHO #ElifeCoinTEST2\r");
        } else {
            // randomly join #ElifeCoin00-#ElifeCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ElifeCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ElifeCoin%02d\r", channel_number).c_str());
        }
